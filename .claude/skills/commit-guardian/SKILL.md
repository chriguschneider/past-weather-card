...
- `dist/weather-station-card.js` updated together with matching `src/` changes — that is *required* by ADR 0001, not a violation.
...
- **`dist/weather-station-card.js` must be in sync.** If `src/**/*.ts` is staged, `dist/weather-station-card.js` must be staged too. CI's `verify dist matches HEAD` step would otherwise fail. Per ADR 0001 this is intentional.
- **No `@ts-nocheck` regressions.** Per ADR 0004, the codebase is fully strict. A new `@ts-nocheck` on a previously-strict file is a finding.
- **No WSL-generated E2E baselines.** Per `CLAUDE.md`, `tests-e2e/snapshots/**.png` may only be regenerated by the GHA `update-baselines.yml` workflow. If the diff touches PNGs and the commit isn't from `github-actions[bot]`, flag it.
